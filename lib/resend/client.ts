...
export async function sendTaxDeadlineAlert(params: {
  ...
  return resend.emails.send({
    from: `Emiday Books <${FROM_EMAIL}>`,
    to: params.to,
    subject: `⚡ Your ${params.taxType} return is due in ${params.daysUntil} days — Emiday Books`,
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #6C3FE8, #8B5CF6); padding: 24px; border-radius: 16px 16px 0 0;">
          <h1 style="color: white; font-size: 20px; margin: 0;">Emiday Books</h1>
        </div>
        <div style="padding: 32px; background: #fff; border: 1px solid #eee; border-radius: 0 0 16px 16px;">
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL}/compliance" style="display: inline-block; background: linear-gradient(135deg, #6C3FE8, #8B5CF6); color: white; padding: 12px 24px; border-radius: 12px; text-decoration: none; font-weight: 600; margin-top: 16px;">View in Emiday Books →</a>
          <p style="color: #A39BBE; font-size: 12px; margin-top: 24px;">This is an automated reminder from Emiday Books.</p>
        </div>
      </div>
    `,
  });
...
export async function sendWelcomeEmail(params: {
  ...
  return resend.emails.send({
    from: `Emiday Books <${FROM_EMAIL}>`,
    to: params.to,
    subject: `Welcome to Emiday Books, ${params.firstName} 🎉`,
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #6C3FE8, #8B5CF6); padding: 24px; border-radius: 16px 16px 0 0;">
          <h1 style="color: white; font-size: 20px; margin: 0;">Welcome to Emiday Books!</h1>
        </div>
        <div style="padding: 32px; background: #fff; border: 1px solid #eee; border-radius: 0 0 16px 16px;">
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard" style="display: inline-block; background: linear-gradient(135deg, #6C3FE8, #8B5CF6); color: white; padding: 12px 24px; border-radius: 12px; text-decoration: none; font-weight: 600;">Go to Dashboard →</a>
        </div>
      </div>
    `,
  });
...
export async function sendAccountantInvite(params: {
  ...
  return resend.emails.send({
    from: `Emiday Books <${FROM_EMAIL}>`,
    to: params.to,
    subject: `${params.senderName} has invited you to manage ${params.businessName} on Emiday Books`,
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #6C3FE8, #8B5CF6); padding: 24px; border-radius: 16px 16px 0 0;">
          <h1 style="color: white; font-size: 20px; margin: 0;">Emiday Books</h1>
        ...
        <div style="padding: 32px; background: #fff; border: 1px solid #eee; border-radius: 0 0 16px 16px;">
          ...
          <p style="color: #6B6280;">${params.senderName} has invited you to manage <strong>${params.businessName}</strong> on Emiday Books.</p>
          <a href="${params.inviteUrl}" style="display: inline-block; background: linear-gradient(135deg, #6C3FE8, #8B5CF6); color: white; padding: 12px 24px; border-radius: 12px; text-decoration: none; font-weight: 600; margin-top: 16px;">Accept Invitation →</a>
        </div>
      </div>
    `,
  });
...
export async function sendWeeklySummary(params: {
  ...
  return resend.emails.send({
    from: `Emiday Books <${FROM_EMAIL}>`,
    to: params.to,
    subject: `Your week in numbers — Emiday Books`,
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #6C3FE8, #8B5CF6); padding: 24px; border-radius: 16px 16px 0 0;">
        ...
        <div style="padding: 32px; background: #fff; border: 1px solid #eee; border-radius: 0 0 16px 16px;">
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard" style="display: inline-block; background: linear-gradient(135deg, #6C3FE8, #8B5CF6); color: white; padding: 12px 24px; border-radius: 12px; text-decoration: none; font-weight: 600; margin-top: 16px;">View Dashboard →</a>
        </div>
      </div>
    `,
  });
